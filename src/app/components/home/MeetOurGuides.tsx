...
// GuideCard component remains the same as it receives props
const GuideCard = ({ name, expertise, languages, imgSrc }: { 
  name: string, 
  expertise: string, 
  languages: string[], 
  imgSrc: string 
  ...
  <div className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 border border-amber-100">
    <div className="h-48 bg-amber-100 overflow-hidden relative">
      <Image 
        src={imgSrc} 
        alt={name} 
        layout="fill" 
...
export const MeetOurGuides: React.FC = () => {
  ...
  const content = {
    header: {
      ...
      subtitle: "Your personal connection to authentic Moroccan experiences"
    },
    guides: [
      {
        name: "Name Here",
    ...
    footer: {
      text: "All our Polish-speaking guides are certified professionals with extensive local knowledge",
      buttonText: "Learn About Our Guide Training"
    }
  };
